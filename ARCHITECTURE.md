...
## Guida allo Sviluppo (Prompt Base)

Quando chiedi a un AI di creare una nuova app per Moca Hub, usa questo prompt base:

> "Stai creando una 'Moca Satellite App'.
> 1. NON implementare login/registrazione. Assumi che `supabase.auth.getSession()` restituisca già un utente loggato dall'Hub.
> 2. Usa Supabase URL/Key forniti nelle env vars.
> 3. Per le API Key (es. OpenAI), non chiederle all'utente. Chiama la funzione Edge `get-client-config` passando la chiave necessaria.
> 4. Stile: Usa TailwindCSS con colori Moca (#E52217 primary, #191919 black).
> 5. Database: Se serve salvare dati, crea tabelle con RLS abilitato che controllano `auth.uid()`."
